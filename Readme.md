...
- Синхронизфция комментариев и ворклогов.
...
**Внимание!** После удаления плагина всё данные плагина (задачи на 
синхронизацию, маппинги и т.д.) удаляется, уже синхронизированные тикеты не удаляются.

![plugin_installation_4](./images/install4.jpg )

## Настройка

Плагин предназначен для автоматизированной синхронизации задач JIRA – JIRA. На данный момент поддерживается синхронизация задач любого типа в тип Task из внешних JIRA (JIRA-источники) в JIRA с установленным плагином (JIRA-приемник).
Предусмотрено отключение синхронизации отдельно комментариев и отдельно ворклогов.

Дополнительно предусмотрено оповещение по email об успешном или неуспешном проведении синхронизации по расписанию.
...
Доступ к плагину осуществляется через меню Administration, раздел Applications. В списке слева выберите Sync Plugin, по умолчанию открывается список задач на синхронизацию.

Для того чтобы начать перенос данных из JIRA-источников в JIRA-приемник нужно создать задачи на синхронизацию и настроить маппинг пользователей.
...
- Task name - может быть любым, но обязательно уникальным (не может быть 2х задач с одинаковыми названиями).
...
- Comment - комментарий, не обязательное поле.
...
- Enabled - показывает включена или нет задача. Если чек-бокс не выбран, то задача сохраниться, но не будет запускаться, в любой момент задачу можно включить или выключить зайдя в режим редактирования задачи.
...
2. Кнопкой удаления в списке задач в разделе "Task llist".
...
- Source user - указать username пользователя внешней JIRA (username можно посмотреть в разделе User managment администрирования внешней JIRA)
...
- Target user - username соответствующего пользователь во внутреннем трекере
...
**Внимание!**
Плагин выдаст ошибку, если при внесении изменений в существующий маппинг в sourse user будет указан пользователь для которого уже есть маппинг.
...
Для того чтобы настроить оповещения о синхронизации нужно перейти в раздел "Notifications" верхней навигационной панели плгина.
По аналогии с маппингом в этом разделе отображается форма создания задач на оповещение и список существующих задач.
...
Send errors only - получать только отчеты об ошибках в синхронизации, если чек-бокс отмечен; получать отчеты и об ошибках и об успешных синхронизациях, если чек-бокс не отмечен.
...
Задача на синхронизацию переносит данные только тех тикетов, которые попали в фильтрацию заданную в задаче (поле Filter - выражение в формате JQL).
...
- Initial task - количество тикетов которые попали в фильтрацию задачи на 
  синхронизацию
- Issues to add - количество тикетов которые синхронизируюся задачей в первый 
